...
export function recallForPool(poolAddress) {
  ...
  if (entry.total_deploys > 0) {
    ...
    const lastCloseReason = lastDeploy?.close_reason ? `, closed because: "${lastDeploy.close_reason}"` : "";
    const lastClosePnl = lastDeploy?.pnl_pct != null ? `, last PnL: ${lastDeploy.pnl_pct >= 0 ? "+" : ""}${lastDeploy.pnl_pct}%` : "";
    const lastHeld = lastDeploy?.minutes_held != null ? `, held: ${lastDeploy.minutes_held}min` : "";
    const lastRangeEff = lastDeploy?.range_efficiency != null ? `, in-range: ${lastDeploy.range_efficiency}%` : "";
    lines.push(`POOL MEMORY [${entry.name}]: ${entry.total_deploys} past deploy(s), avg PnL ${entry.avg_pnl_pct}%, win rate ${entry.win_rate}%, last outcome: ${entry.last_outcome}${lastClosePnl}${lastCloseReason}${lastHeld}${lastRangeEff}`);
  }
